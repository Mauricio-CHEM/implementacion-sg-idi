...
def gh_load():
    if not GH_CONFIGURED:
        return None, None
    try:
        url = ("https://api.github.com/repos/" + GH_REPO
...
def gh_save(state, sha=None):
    ...
    try:
        ...
        headers = {"Authorization": "token " + GH_TOKEN,
                   "Accept": "application/vnd.github.v3+json"}
        payload = {"message": "chore: estado SGI " + datetime.now().strftime("%Y-%m-%d %H:%M"),
...
PHASES = {
    ...
    "Fase 3": {
        ...
        "items": [
            ...
            {"id":"3.9","activity":"Formato formulacion de proyecto","ref":"NTC5802","responsible":"Lider SGI","deadline":"Mes 8","evidence":"FTO-19"},
...
PHASE_PAGES = {
    ...
    "Fase 2 - Apoyo Estrategico":   "Fase 2",
    "Fase 3 - Operacion":           "Fase 3",
    "Fase 4 - Evaluacion y Mejora": "Fase 4",
}
...
def get_istate(key):
    ...
    return {"status": v.get("status","Pendiente"),
            "fecha_inicio": v.get("fecha_inicio",""),
            "fecha_fin": v.get("fecha_fin",""),
            "responsable_nombre": v.get("responsable_nombre",""),
            "rol": v.get("rol",""),
...
.det{background:#F8FAFF;border:1px solid #E3EAF8;border-radius:10px;padding:12px 16px}
</style>""", unsafe_allow_html=True)

# ── SIDEBAR ───────────────────────────────────────────────────────────────────
with st.sidebar:
    logo_b64 = st.session_state.state.get("logo_b64")
    if logo_b64:
    ...
    if GH_CONFIGURED and not st.session_state.gh_loaded:
    ...
    if GH_CONFIGURED:
        ...
        st.markdown(f'<span style="background:{color};color:white;padding:3px 10px;'
                    f'border-radius:10px;font-size:.74rem;font-weight:600">{label}</span>',
                    unsafe_allow_html=True)
    ...
    if GH_CONFIGURED:
        ...
        with c2:
            if st.button("Guardar", use_container_width=True, type="primary"):
                with st.spinner("..."):
                    ok, msg, sha2 = gh_save(st.session_state.state, st.session_state.gh_sha)
                if ok:
    ...
    st.download_button("Descargar JSON",
        ...
        file_name="sgi_state_" + datetime.now().strftime("%Y%m%d_%H%M") + ".json",
...
if page == "Dashboard":
    ...
    if GH_CONFIGURED:
        ...
        if src == "github":
            st.info(f"Sincronizado con GitHub  SHA: {sha_s}")
        else:
            st.warning("Estado local. Presiona Guardar para sincronizar.")
    st.divider()
    ...
    for i,pk in enumerate(PHASES):
        ...
        card = (f"<div class='kcard' style='border-left-color:{ph['color']}'>"
                f"<div style='font-weight:700'>{pk}</div>"
                f"<div style='font-size:.8rem;color:#777'>{ph['name']}</div>"
                f"<div class='kpct' style='color:{ph['color']}'>{pct}%</div>"
                f"<div style='font-size:.8rem;color:#777'>{ph['months']}</div>"
                f"<div style='margin-top:8px;font-size:.82rem'>"
                f"OK {done} | ~ {wip} | P {pend} | N/A {na}</div></div>")
        with cols[i]:
    ...
    with c1:
        ...
        fig = px.bar(pd.DataFrame(cr),x="Fase",y="n",color="Estado",barmode="stack",height=300,
                     color_discrete_map={"Completo":"#4CAF50","En proceso":"#2196F3",
    ...
    with c2:
        ...
        lbls=[f"F{i+1}: "+PHASES[pk]["name"][:14] for i,pk in enumerate(PHASES)]
    ...
    for i,pk in enumerate(PHASES):
        ...
        hito=(f"<div style='background:{ph['color']}12;border:1px solid {ph['color']}40;"
              f"border-radius:10px;padding:14px;text-align:center'>"
              f"<div style='color:{ph['color']};font-weight:700'>{MES_HITO[pk]}</div>"
              f"<div style='font-size:1.3rem;margin:6px 0'>{ico}</div>"
              f"<div style='font-size:.8rem;color:#444'>{ph['hito']}</div></div>")
...
# ── CHECKLIST PAGES ───────────────────────────────────────────────────────────
    ...
    st.divider()
    kc=st.columns(5)
    kc[0].metric("Avance",f"{pct}%"); kc[1].metric("Completas",done)
    kc[2].metric("En proceso",wip);   kc[3].metric("Pendientes",total-done-wip-na)
    kc[4].metric("No aplica",na)
    ...
    for col,lbl in zip(hdr,["**#**","**Ref.**","**Actividad**","**Plazo**","**Estado**"]):
        col.markdown(lbl)
    ...
    for item in ph["items"]:
        ...
        if ist["fecha_inicio"]:       badges+=f" | {ist['fecha_inicio']}"
        if ist["fecha_fin"]:          badges+=f" -> {ist['fecha_fin']}"
        row[2].markdown(f"{item['activity']}<br><small style='color:#888'>{badges}</small>",
                        unsafe_allow_html=True)
        ...
        if new_st!=cur:
            ist["status"]=new_st; save_istate(k,ist); st.rerun()
        with st.expander(f"Detalles  {item['id']}: {item['activity'][:55]}"):
            ...
            new_cmt =st.text_area("Comentario / Enlace",value=ist["comentario"],
                                   key="cm_"+pk+"_"+item["id"],height=70)
            if ist["comentario"] and ist["comentario"].startswith("http"):
                st.markdown(f'<a href="{ist['comentario']}" target="_blank">Ver evidencia</a>',
                            unsafe_allow_html=True)
            st.caption(f"Evidencia esperada: {item['evidence']}")
    ...
    for tab,items_list,tid in [(t1,DOCUMENTS,"docs"),(t2,FORMATS,"fmts")]:
        with tab:
            ...
            for d in items_list:
                ...
                tag=f"<span class='tag' style='background:{color}20;color:{color}'>{d['phase']}</span>"
                ...
                if edit_codes:
                    ...
                    row[1].markdown(f"**`{cc}`**" if cc!=d["code"] else f"`{cc}`")
                row[2].markdown(f"<small>`{d.get('chapter','-')}`</small>",unsafe_allow_html=True)
                row[3].markdown(f"{d['name']}<br><small style='color:#999'>{d.get('type','-')}</small>",
                                unsafe_allow_html=True)
...
# ── REPORTES ──────────────────────────────────────────────────────────────────
    ...
    for pk,ph in PHASES.items():
        for item in ph["items"]:
            ...
            rows.append({"Fase":pk,"ID":item["id"],"Actividad":item["activity"],
                         ...
                         "Responsable":ist["responsable_nombre"],
                         "Estado":ist["status"],"Comentario":ist["comentario"]})
    df=pd.DataFrame(rows)
    st.markdown("### Resumen por Fase")
    summary=[]
    for pk in PHASES:
        total,done,wip,na,pct=phase_progress(pk)
        summary.append({"Fase":pk+": "+PHASES[pk]["name"],"Total":total,
                         "Completas":done,"En proceso":wip,
                         "Pendientes":total-done-wip-na,"N/A":na,"% Avance":f"{pct}%"})
    st.dataframe(pd.DataFrame(summary),use_container_width=True,hide_index=True)
    st.divider()
    dfp=df[df["Estado"].isin(["En proceso","Pendiente"])]
    st.markdown(f"### Pendientes / En proceso ({len(dfp)})")
    ...
    st.download_button("Descargar CSV",df.to_csv(index=False).encode(),
        "SGI_"+datetime.now().strftime("%Y%m%d")+".csv","text/csv",use_container_width=True)
...
# ── CONFIGURACION ─────────────────────────────────────────────────────────────
    ...
    if GH_CONFIGURED:
        st.success(f"Repo: {GH_REPO} | {GH_PATH} | {GH_BRANCH}")
...
GITHUB_BRANCH    = 'main'",language="toml")
    ...
    with c1:
        if lb: st.markdown(f'<img src="{lb}" style="width:100%">',unsafe_allow_html=True)
    ...
    st.markdown(f"**Version:** 4.0 | **Actividades:** {n} | **Docs:** 45 | **Formatos:** 30")
