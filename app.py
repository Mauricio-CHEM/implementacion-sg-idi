...
with st.sidebar:
    ...
    st.caption(f"{done_all} de {applicable_all} actividades aplicables completadas")
    st.markdown("---")
...
if page == "🏠 Dashboard":
    ...
    with c2:
        ...
        labels = [f"F{i+1}: {PHASES[pk]['name'][:20]}" for i, pk in enumerate(PHASES)]
        ...
        fig_r.update_layout(polar=dict(radialaxis=dict(range=[0, 100], tickfont_size=10)),
...
# ============================================================
    ...
    for item in ph["items"]:
        ...
        if current not in f_status:
            continue
        if f_text and f_text.lower() not in item["activity"].lower():
            continue
        if item["responsible"] not in f_resp:
            continue
...
# ============================================================
    ...
    st.markdown("Inventario de los **45 documentos base** y **30 formatos operativos**. Actualiza el estado a medida que avances.")
    st.divider()
...
# ============================================================
    ...
    st.markdown("### 📅 Distribución de actividades por mes")
    month_order = [f"Mes {i}" for i in range(1, 13)]
    df_all["Mes_num"] = df_all["Plazo"].str.extract(r"(\d+)").astype(int)
    ...
    if df_pending.empty:
        st.success("🎉 ¡Todas las actividades están completas o marcadas como no aplica!")
    else:
    ...
    with e1:
        ...
        st.download_button("⬇️ Descargar lista de actividades (CSV)", csv,
                           f"SGI_actividades_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv",
    ...
    with e2:
        ...
        for d in DOCUMENTS + FORMATS:
            k = doc_key(d["code"])
            doc_rows.append({
                "Código": d["code"], "Nombre": d["name"], "Fase": d["phase"],
                "Estado": st.session_state.state.get(k, "⬜ Pendiente")
            })
        csv_docs = pd.DataFrame(doc_rows).to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Descargar registro documental (CSV)", csv_docs,
                           f"SGI_documentos_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv",
                           use_container_width=True)
