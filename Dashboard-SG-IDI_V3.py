...
# requests es opcional — si no está instalado, GitHub Sync se deshabilita
try:
    import requests
    _REQUESTS_OK = True
except ImportError:
...
# ── GitHub Sync config — completamente seguro ante cualquier error ──────────
...
def gh_load():
    """Carga el estado desde GitHub. Retorna (dict, sha) o (None, None)."""
...
def gh_save(state, sha=None):
    """Sube el estado a GitHub. Retorna (ok, mensaje, nuevo_sha)."""
    if not GH_CONFIGURED:
    ...
    try:
        ...
        return False, r.json().get("message", f"HTTP {r.status_code}"), None
