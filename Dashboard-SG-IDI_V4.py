    ...
    if GH_ON:
        ...
        with c2:
            if st.button('Guardar', use_container_width=True, type='primary'):
                ...
                if ok:
                    st.session_state.gh_sha = sha2; st.session_state.gh_source = 'github'; st.success(msg)
                else: st.error(msg)
        st.markdown('---')
...
# ── Checklist pages ───────────────────────────────────────────────────────
    ...
    st.divider()
    kc = st.columns(5)
    kc[0].metric('Avance',     str(pct) + '%')
    kc[1].metric('Completas',  done)
    kc[2].metric('En proceso', wip)
    kc[3].metric('Pendientes', total - done - wip - na)
    ...
    for item in ph['items']:
        ...
        row[2].markdown(item['activity'] + '<br><small style="color:#888">' + badges + '</small>',
                        unsafe_allow_html=True)
        ...
        with st.expander('Detalles  ' + item['id'] + ': ' + item['activity'][:55]):
            ...
            if ist['comentario'] and ist['comentario'].startswith('http'):
                st.markdown('<a href="' + ist['comentario'] + '" target="_blank">Ver evidencia</a>',
                            unsafe_allow_html=True)
    ...
    for tab, items_list, tid in [(t1, DOCUMENTS, 'docs'), (t2, FORMATS, 'fmts')]:
        with tab:
            ...
            for d in items_list:
                ...
                row[3].markdown(d['name'] + '<br><small style="color:#999">' + d.get('type', '-') + '</small>',
                                unsafe_allow_html=True)
...
# ── Reportes ──────────────────────────────────────────────────────────────
    ...
    for pk in PHASES:
        ...
        summary.append({'Fase': pk + ': ' + PHASES[pk]['name'], 'Total': total,
                         ...
                         'Pendientes': total - done - wip - na, 'N/A': na, '% Avance': str(pct) + '%'})
    ...
    st.download_button('Descargar CSV',
        ...
        file_name='SGI_' + datetime.now().strftime('%Y%m%d') + '.csv',
...
# ── Configuracion ─────────────────────────────────────────────────────────
    ...
    if GH_ON:
        ...
        st.caption('SHA: ' + (st.session_state.gh_sha or 'no cargado')[:7]
                   + ' | ' + st.session_state.get('gh_source', 'local'))
    else:
        st.warning('GitHub Sync no configurado. Agrega en Settings > Secrets:')
    ...
    st.markdown('**Version:** 4.1 | **Actividades:** ' + str(n) + ' | **Docs:** 45 | **Formatos:** 30')
